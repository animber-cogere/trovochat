...
let conn = async { trovochat::connect_tls(&user_config).await.unwrap() };
```
> Connect with default capabilities using just a username and oauth token:
```rust no_run
let my_oauth = std::env::var("MY_TROVO_OAUTH").unwrap();
// the conn type is an tokio::io::{AsyncRead + AsyncWrite}
let conn = async { trovochat::connect_easy_tls("my_name", &my_oauth).await.unwrap() };
```
...
let user_config = trovochat::UserConfig::builder().build().unwrap();
// the conn type is an tokio::io::{AsyncRead+AsyncWrite}
let conn = async { trovochat::connect(&user_config).await.unwrap() };
...
let my_oauth = std::env::var("MY_TROVO_OAUTH").unwrap();
// the conn type is an tokio::io::{AsyncRead+AsyncWrite}
let conn = async { trovochat::connect_easy("my_name", &my_oauth).await.unwrap() };
```
...
let conn = async { trovochat::connect_easy_tls(nick, pass).await.unwrap() };
```

> Without TLS
```rust no_run
let (nick, pass) = trovochat::ANONYMOUS_LOGIN;
let conn = async { trovochat::connect_easy(nick, pass).await.unwrap() };
```
...
let (_runner, mut control) = trovochat::Runner::new(trovochat::Dispatcher::new(), trovochat::RateLimit::default());
...
```rust no_run
use tokio::stream::StreamExt as _;
use trovochat::{events, messages, Control, Dispatcher, IntoChannel, Runner, Status, RateLimit, Writer};
