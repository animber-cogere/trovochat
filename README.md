...
## Connecting to Trovo
This crate allows you connect to Trovo with a TLS stream, or without one.
...
let user_config = trovochat::UserConfig::builder().build().unwrap();
// the conn type is an tokio::io::{AsyncRead + AsyncWrite}
let conn = async { trovochat::native_tls::connect(&user_config).await.unwrap() };
```
> Connect with default capabilities using just a username and oauth token:
```rust no_run
let my_oauth = std::env::var("MY_TROVO_OAUTH").unwrap();
// the conn type is an tokio::io::{AsyncRead + AsyncWrite}
let conn = async { trovochat::native_tls::connect_easy("my_name", &my_oauth).await.unwrap() };
```
...
let user_config = trovochat::UserConfig::builder().build().unwrap();
// the conn type is an tokio::io::{AsyncRead+AsyncWrite}
let conn = async { trovochat::connect_no_tls(&user_config).await.unwrap() };
...
let my_oauth = std::env::var("MY_TROVO_OAUTH").unwrap();
// the conn type is an tokio::io::{AsyncRead+AsyncWrite}
let conn = async { trovochat::connect_easy_no_tls("my_name", &my_oauth).await.unwrap() };
```
...
let conn = async { trovochat::native_tls::connect_easy(nick, pass).await.unwrap() };
```

> Without TLS
```rust no_run
let (nick, pass) = trovochat::ANONYMOUS_LOGIN;
let conn = async { trovochat::connect_easy_no_tls(nick, pass).await.unwrap() };
```
...
See [`trovochat::sync`](https://docs.rs/trovochat/latest/trovochat/sync/index.html) for synchronous types.

```rust no_run
// make a user config, builder lets you configure it.
let user_config = trovochat::UserConfig::builder().build().unwrap();
let conn: std::net::TcpStream = trovochat::sync::connect(&user_config).unwrap();
```
Or
```rust no_run
let my_oauth = std::env::var("MY_TROVO_OAUTH").unwrap();
let conn: std::net::TcpStream = trovochat::sync::connect_easy("my_name", &my_oauth).unwrap();
```
---

## Parsing messages

Parsing is done with the `decode(&str)` or `decode_one(&str)` methods. Trovo (IRC) messages are delimited by `CRLF [0xD, 0xA]`
...
for msg in trovochat::decode(&input).flatten() {
    // msg is a trovochat::decode::Message<'a>
    ...
    use trovochat::color::{Color, TrovoColor, RGB};
...
let (d, message) = trovochat::decode_one(input).unwrap();
assert!(d > 0);
assert_eq!(message.command, "PING");

// use the new index
let (i, message) = trovochat::decode_one(&input[d..]).unwrap();
assert_eq!(i, 0);
...
use trovochat::Parse as _;
use trovochat::{decode, messages};

let input = ":museun!museun@museun.tmi.trovo.tv JOIN #some_test_channel\r\n";
let msg: decode::Message<'_> = trovochat::decode(input).next().unwrap().unwrap();
...
Along with connecting to trovo and parsing strings, this crate can do that for you and provide you typed asynchronous Streams for events you're interested in.

```rust no_run
use trovochat::{messages, events};
...
let (_runner, mut control) = trovochat::Runner::new(trovochat::Dispatcher::new());
...
```rust
use std::io::Cursor;
use trovochat::encode::AsyncEncoder;
...
```rust
use std::io::Cursor;
// or get it from trovochat:sync::Encoder;
use trovochat::encode::Encoder;
...
```rust no_run
use tokio::stream::StreamExt as _;
use trovochat::{events, messages, Control, Dispatcher, IntoChannel, Runner, Status, Writer};

fn get_user_pass() -> (String, String) {
    (std::env::var("TROVO_NICK").unwrap(), std::env::var("TROVO_PASS").unwrap())
}
...
async fn main() {
    ...
    let connector = trovochat::Connector::new(|| async move {
        ...
        trovochat::native_tls::connect_easy(&user, &pass).await
    });
