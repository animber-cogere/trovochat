...
use trovochat::{commands, connector, messages, rate_limit::NullBlocker, runner::Status};

fn expect_env_var(key: &str) -> String {
    std::env::var(key).unwrap_or_else(|_| panic!("please set `{}`", key))
}

fn get_user_config() -> trovochat::UserConfig {
    let name = expect_env_var("TROVO_NAME");
    let token = expect_env_var("TROVO_TOKEN");

    // you need a UserConfig to connect to Trovo
    trovochat::UserConfig::builder()
...
async fn main() {
    ...
    // create a connector using `async_std`, this connects to Trovo.
    // you can provide a different address with `custom`
    let connector = connector::async_std::Connector::trovo();

    // create a new Dispatcher. this allows you to 'subscribe' to specific events
    let dispatcher = trovochat::AsyncDispatcher::new();
    ...
    let mut runner = trovochat::AsyncRunner::new(dispatcher, user_config, connector);
    ...
    match res {
        ...
        // this is the normal exit. Trovo closed your connection.
