...
This crate produces types for trovo messages and gives you methods for sending messages.

# 'Theory' of operation
First, by creating a [`Client`](./struct.Client.html) from a [`ReadAdapter`](./trait.ReadAdapter.html) and a
[`WriteAdapter`](./trait.WriteAdapter.html). This can be simple be done by using [`trovochat::sync_adapters`](./fn.sync_adapters.html)
...
filled-out [`UserConfig`](./userconfig/struct.UserConfig.html) will connect you to Trovo.

Once connected, you can [`Client::wait_for_ready`](./struct.Client.html#method.wait_for_ready) and the client will read **(blocking)** until Trovo sends a
...
use trovochat::{commands::PrivMsg, Capability};
use trovochat::{Client, Writer, SyncReadAdapter, SyncWriteAdapter};
use trovochat::{TROVO_IRC_ADDRESS, UserConfig};
# fn main() {
// create a simple TcpStream
let read = TcpStream::connect(TROVO_IRC_ADDRESS).expect("connect");
let write = read
    .try_clone()
