...
use trovochat::{events, Control, Dispatcher, Runner, Status};

#[tokio::main]
async fn main() {
    let channels = &[std::env::var("TROVO_CHANNEL").unwrap()];

    let dispatcher = Dispatcher::new();
    let (runner, control) = Runner::new(dispatcher.clone(), trovochat::RateLimit::default());
