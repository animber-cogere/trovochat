/*!
Color types associated with Trovo.
...
assert_eq!(color.kind, TrovoColor::BlueViolet);
```
...
// RGB can be converted into a TrovoColor
let trovo_color: TrovoColor = rgb.into();

// TrovoColor can be converted into an RGB
let rgb: RGB = trovo_color.into();
```
*/
...
/**
A trovo color paired with an RGB

Trovo has named colors. Users with `Turbo` enabled accounts can set a custom color.
...
impl From<RGB> for TrovoColor {
    fn from(rgb: RGB) -> Self {
        trovo_colors()
            .iter()
            .find(|(_, color)| *color == rgb)
            .map(|&(color, _)| color)
            .unwrap_or_else(|| TrovoColor::Turbo)
    }
}

impl From<TrovoColor> for RGB {
    fn from(color: TrovoColor) -> Self {
        trovo_colors()
            .iter()
            .find(|(c, _)| *c == color)
            .map(|&(_, rgb)| rgb)
...
mod tests {
    ...
    fn parse_turbo_color() {
        ...
        assert_eq!(color.kind, TrovoColor::Turbo);
