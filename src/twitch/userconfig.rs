...
/**
User configuration for 'registering' with Trovo
...
```
# use trovochat_sync::trovo::{Capability, UserConfig};
# std::env::set_var("TROVO_NAME", "foo");
# std::env::set_var("TROVO_TOKEN", format!("oauth:{}", "a".repeat(30)));
// as anonymous
let config = UserConfig::builder().anonymous().build().unwrap();
// or with a name/token
let name = std::env::var("TROVO_NAME").unwrap();
let token = std::env::var("TROVO_TOKEN").unwrap();
