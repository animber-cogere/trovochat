...
/// Client is the IRC client for interacting with Trovo's chat.
// TODO write usage
    ...
    W: Write,
{
    ...
    /// Usage
    /// ```no_run
    /// # use trovochat::{TestStream, trovo::Client, UserConfig};
    ...
    /// Usage:
    /// ```no_run
    /// # use trovochat::{TestStream, trovo::Client};
    /// # let mut stream = TestStream::new();
    /// # let (r, w) = (stream.clone(), stream.clone());
    /// # let mut client = Client::new(r, w);
...
pub trait ClientExt {
    ...
    ///
    /// ```no_run
    /// # use trovochat::{TestStream, trovo::Client, trovo::ClientExt};
    /// # let mut stream = TestStream::new();
    /// # let (r, w) = (stream.clone(), stream.clone());
    /// # let mut client = Client::new(r, w);
