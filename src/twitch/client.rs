    ...
    R: Read + Sync + Send,
{
    ...
    /// - If this is not set and the `tags` capability was set, then a [`Event::TrovoReady`](enum.Event.html#variant.TrovoReady) is produced instead
    ///
    /// It will try to be smart and produce a `TrovoReady` event if desired, otherwise `IrcReady` was produced
...
pub enum Event {
    ...
    IrcReady(String),
    /// A trovo ready event was requested, returning your trovo user
    TrovoReady(LocalUser),
    /// A trovo message
    Message(Message),
    /// An error
    Error(Error),
                ...
                match &msg {
                    Message::Irc(msg) => match &**msg {
                        crate::irc::Message::Cap {
                            ...
                            "trovo.tv/tags" => self.caps.push(Capability::Tags),
                            "trovo.tv/membership" => self.caps.push(Capability::Membership),
                            "trovo.tv/commands" => self.caps.push(Capability::Commands),
                            _ => {}
                        },
                        _ => {}
                    },
            ...
            ClientState::TrovoReady => loop {
                ...
                match read!() {
                    ...
                    Message::GlobalUserState(state) => {
                        self.state.next(ready);
                        return Some(Event::TrovoReady(LocalUser {
...
impl ClientState {
    fn next(&mut self, ready: ReadyState) {
        let _ = match self {
            ...
            ClientState::Start => std::mem::replace(self, ClientState::TrovoReady),
