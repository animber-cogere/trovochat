use crate::{register::register, simple_user_config, UserConfig, TROVO_IRC_ADDRESS};

#[cfg(any(feature = "tokio_native_tls", feature = "tokio_rustls"))]
use crate::TROVO_IRC_ADDRESS_TLS;
...
pub async fn connect(user_config: &UserConfig) -> std::io::Result<TokioStream> {
    let mut stream = tokio::net::TcpStream::connect(TROVO_IRC_ADDRESS).await?;
...
pub async fn connect_tls(user_config: &UserConfig) -> std::io::Result<TokioTlsStream> {
    let stream = tokio::net::TcpStream::connect(TROVO_IRC_ADDRESS_TLS).await?;
