...
/// # NOTE:
///
/// Because Trovo is extremely inconsistent in its documentation you can get this message without any Tags attached.
///
...
mod tests {
    ...
    fn global_user_state_no_tags() {
        ...
        for msg in parse(input).map(|s| s.unwrap()) {
            ...
            assert_eq!(msg.color(), crate::trovo::Color::default());
            assert_eq!(msg.emote_sets(), vec!["0"]);
        }
    }
    ...
    fn global_user_state_empty_color() {
        let input = "@badge-info=;badges=;color=;display-name=shaken_bot;emote-sets=0;user-id=241015868;user-type= :tmi.trovo.tv GLOBALUSERSTATE\r\n";
        for msg in parse(input).map(|s| s.unwrap()) {
            let msg = GlobalUserState::from_irc(msg).unwrap();
            assert_eq!(msg.user_id().unwrap(), "241015868");
            assert_eq!(msg.display_name().unwrap(), "shaken_bot");
