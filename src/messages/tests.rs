...
#[test]
fn raw() {
    let input = ":museun!museun@museun.tmi.trovo.tv PRIVMSG #museun :testing over here\r\n";
...
#[test]
fn user_notice() {
    let input = ":tmi.trovo.tv USERNOTICE #museun\r\n";
    for msg in crate::decode(input).map(|s| s.unwrap()) {
        assert_eq!(
            UserNotice::parse(&msg).unwrap(),
            UserNotice {
...
fn names_start() {
    let input =
        ":museun!museun@museun.tmi.trovo.tv 353 museun = #museun :shaken_bot4 shaken_bot5\r\n";
    for msg in crate::decode(input).map(|s| s.unwrap()) {
        assert_eq!(
...
#[test]
fn join_bad_command() {
    let input = crate::decode(":tmi.trovo.tv NOT_JOIN #foo\r\n".into())
        .flatten()
        .next()
        .unwrap();
...
#[test]
fn join_bad_nick() {
    let input = crate::decode(":tmi.trovo.tv JOIN #foo\r\n".into())
        .flatten()
        .next()
        .unwrap();
...
#[test]
fn join_bad_channel() {
    let input = crate::decode(":tmi.trovo.tv JOIN\r\n".into())
        .flatten()
        .next()
        .unwrap();
...
#[test]
fn ready() {
    let input = ":tmi.trovo.tv 376 shaken_bot :>\r\n";
    for msg in crate::decode(input).map(|s| s.unwrap()) {
        assert_eq!(
