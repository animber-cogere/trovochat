...
use crate::{simple_user_config, UserConfig, TROVO_IRC_ADDRESS};
...
pub fn connect(user_config: &UserConfig) -> std::io::Result<TcpStream> {
    let mut stream = TcpStream::connect(TROVO_IRC_ADDRESS)?;
...
# Example
```rust,no_run
# use trovochat::{sync::*, ANONYMOUS_LOGIN};
