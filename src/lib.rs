...
//! produce messages that trovo understands.
//!
...
//! the trovo module
//!
//! The [`trovo`](./trovo/index.html) module contains many data types that encapsulate the various
//! functionality of the irc-portion of Trovo's chat system
//!
//! # 'Theory' of operation
//! First, by creating a [`Client`](./trovo/struct.Client.html) from a Read/Write pair
//! (such as a cloned TcpStream) then calling
//! [`Client::register`](./trovo/struct.Client.html#method.register) with a filled-out
//! [`UserConfig`](./struct.UserConfig.html) will connect you to Trovo. Once
//! connected, you can
...
//! - use [`Client::join`](./trovo/struct.Client.html#method.join) to join a channel.
//! - use [`Client::on`](./trovo/struct.Client.html#method.on) to set up a message filter.
//! - use [`Client::read_message`](./trovo/struct.Client.html#method.read_message) to read a message (and pump the filters).
...
//!
//! To use it, you simply just `register` a closure with the client via its [`Client::on`](./trovo/struct.Client.html#method.on) method.
//! It uses the type of the closures argument, one of [*these*](./trovo/commands/index.html#structs) to create a filter
//! When
...
//! to the callback. Because it does this on same thread as the [`Client::read_message`](./trovo/struct.Client.html#method.read_message)
//! call, you can lock up the system by simplying diverging.
//!
//! The client is thread safe, and clonable so one could call  [`Client::read_message`](./trovo/struct.Client.html#method.read_message)
...
//! let (nick, pass) = (env!("MY_TROVO_OAUTH_TOKEN"), "my_name");
...
//! internal buffers, allowing you to easily write unit tests for the [`Client`](./trovo/struct.Client.html)
