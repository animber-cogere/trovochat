...
/// Simple function to connect to Trovo using a [`TcpStream (non-TLS)`](https://doc.rust-lang.org/std/net/struct.TcpStream.html)
///
/// This is a convenience function for doing the same as:
/// ```no_run
/// use std::net::TcpStream;
/// let conn = std::net::TcpStream::connect(trovochat::TROVO_IRC_ADDRESS).unwrap();
/// let (read, write) = (conn.try_clone().unwrap(), conn);
///
/// let client = trovochat::Client::register(
///     trovochat::UserConfig::with_caps()
///         .nick("your_name_here")
...
{
    let (read, write) = {
        let stream = std::net::TcpStream::connect(TROVO_IRC_ADDRESS).map_err(Error::Connect)?;
...
pub const ANONYMOUS_LOGIN: (&str, &str) = (
    trovo::userconfig::JUSTINFAN1234,
    trovo::userconfig::JUSTINFAN1234,
);
