...
This crate simply read lines from an [`std::io::Read`](https://doc.rust-lang.org/std/io/trait.Read.html) and produces data types for the corresponding messages, and takes an [`std::io::Write`](https://doc.rust-lang.org/std/io/trait.Write.html) which can produce messages that trovo understands.

# 'Theory' of operation
First, by creating a [`Client`](./struct.Client.html) from a Read/Write pair (such as a cloned TcpStream) then calling [`Client::register`](./struct.Client.html#method.register) with a filled-out [`UserConfig`](./userconfig/struct.UserConfig.html) will connect you to Trovo. Once connected, you can [`Client::wait_for_ready`](./struct.Client.html#method.wait_for_ready) and the client will read **(blocking)** until Trovo sends a [`GlobalUserState`](./commands/struct.GlobalUserState.html) message, which it'll fill out a [`LocalUser`](./struct.LocalUser.html) with various information.
...
use trovochat::{commands::PrivMsg, Capability, Client, Writer, SyncReadAdapter};
...
(e.g. complete the connection) with Trovo

```no_run
use trovochat::UserConfig;
let my_token = std::env::var("MY_TROVO_OAUTH_TOKEN").unwrap();
...
let trovo_msg = trovochat::Message::parse(msg);
let pm = match trovo_msg {
    trovochat::Message::PrivMsg(pm) => pm,
    _ => unreachable!(),
};
assert_eq!(pm.user(), "museun");
assert_eq!(pm.channel(), "#test_channel");
assert_eq!(pm.message(), "hello world");
assert_eq!(pm.color().unwrap().kind, trovochat::TrovoColor::OrangeRed);
```
**/
