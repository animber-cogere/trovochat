/*! [`tokio-rustls`][rustls] connector for using a TLS connection with Trovo.
...
/// Connect to Trovo using TLS via **rustls**. Using the provided [`UserConfig`][UserConfig].
///
...
/// let mut stream = trovochat::rustls::connect(&user_config).await?;
...
pub async fn connect(config: &UserConfig) -> std::io::Result<Stream> {
    ...
    let domain_name = tokio_rustls::webpki::DNSNameRef::try_from_ascii_str(TROVO_DOMAIN)
        .expect("valid trovo domain dns/ref");
...
/// Connect to Trovo using TLS via **rustls**. Using the provided `name`, `token`.
///
/// This registers with the connection before returning it.
///
/// # Example
...
/// let mut stream = trovochat::rustls::connect_easy(&name, &token).await?;
/// # Ok::<_, Box<dyn std::error::Error>>(())
/// # }).unwrap();
/// ```
