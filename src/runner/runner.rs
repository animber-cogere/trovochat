...
/// A connector type that acts as a factory for connecting to Trovo
            ...
            tokio::select! {
                ...
                Ok(n) = &mut stream.read_line(&mut buffer) => {
                    ...
                    for msg in decode(&buffer) {
                        let msg = msg?;
                        log::trace!(target: "trovochat::runner::read", "< {}", msg.raw.escape_debug());
                        self.dispatcher.dispatch(&msg);
                        visited = true;
                    }

                    // if we didn't parse a message then we should signal that this was EOF
                    // trovo sometimes just stops writing to the client
                    if !visited {
                        log::warn!("trovo sent an incomplete message");
                        break Ok(Status::Eof)
                    }
                    buffer.clear();
                ...
                Some(data) = &mut self.receiver.next() => {
                    log::trace!(target: "trovochat::runner::write", "> {}", std::str::from_utf8(&data).unwrap().escape_debug());
                ...
                _ = timeout_delay.notified() => {
                    log::warn!(target: "trovochat::runner::timeout", "timeout detected, quitting loop");
                    drop(check_timeout);
                    timeout_task.await;
        ...
        loop {
            tokio::select! {
                _ = tokio::time::delay_for(PING_INACTIVITY) => {
                    log::debug!(target: "trovochat::runner::timeout", "inactivity detected of {:?}, sending a ping", PING_INACTIVITY);
                    ...
                    if writer.ping(&format!("{}", ts)).await.is_err() {
                        timeout.notify();
                        log::error!(target: "trovochat::runner::timeout", "cannot send ping");
                        break;
                    }
                    ...
                    {
                        timeout.notify();
                        log::error!(target: "trovochat::runner::timeout", "did not get a ping after {:?}", PING_WINDOW);
                        break;
                    }
                }
                Some(..) = rx.next() => { }
