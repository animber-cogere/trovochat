...
fn parse_user_prefix() {
    let prefix = Prefix::parse(":museun!museun@museun.tmi.trovo.tv").unwrap();
    assert_eq!(
        prefix,
        Prefix::User {
            nick: "museun".into(),
...
fn parse_server_prefix() {
    let prefix = Prefix::parse(":tmi.trovo.tv").unwrap();
    assert_eq!(
        prefix,
        Prefix::Server {
            host: "tmi.trovo.tv".into(),
        },
    );
...
fn missing_colon_prefix() {
    for input in &["museun!museun@museun.tmi.trovo.tv", "not_tmi.trovo.tv"] {
...
#[test]
fn cap_ack() {
    let input = "tmi.trovo.tv CAP * ACK :trovo.tv/commands\r\n";
    ...
    assert_eq!(msg.data.unwrap(), "trovo.tv/commands")
}
